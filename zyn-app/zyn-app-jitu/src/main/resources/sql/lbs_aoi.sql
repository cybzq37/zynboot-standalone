...
-- 通用过滤索引：适合 aoi_type + 主键/分页类查询
CREATE INDEX IF NOT EXISTS idx_lbs_aoi_type_id
    ON lbs_aoi(aoi_type, id);

-- 行政区过滤索引
CREATE INDEX IF NOT EXISTS idx_lbs_aoi_gb_code
    ON lbs_aoi(gb_code);

-- 通用空间索引：适合未区分类型的 bbox / 点查面
CREATE INDEX IF NOT EXISTS idx_lbs_aoi_geom
    ON lbs_aoi USING GIST(geometry);

-- 按类型拆开的空间索引：适合 AOI_BP / AOI_BUILDING 图层独立查询
CREATE INDEX IF NOT EXISTS idx_lbs_aoi_bp_geom
    ON lbs_aoi USING GIST(geometry)
    WHERE aoi_type = 'BP';

CREATE INDEX IF NOT EXISTS idx_lbs_aoi_building_geom
    ON lbs_aoi USING GIST(geometry)
    WHERE aoi_type = 'BUILDING';

-- 中心点索引：适合后续按中心点做轻量范围/近邻查询
CREATE INDEX IF NOT EXISTS idx_lbs_aoi_center_point
